...
export function CompanyKeywordSearch({ filters }: CompanyKeywordSearchProps) {
  ...
  return (
    <form
      ...
      aria-busy={isPending}
      className="grid w-full grid-cols-[minmax(0,1fr)_auto] items-center gap-2 sm:flex sm:flex-row"
    >
      <label className="relative block min-w-0 flex-1">
        ...
        <input
          ...
          placeholder="회사명, 대표자명, 업종, 주요생산품 검색"
      ...
      <button
        type="submit"
        disabled={isPending}
        className="inline-flex h-10 min-w-20 items-center justify-center gap-2 rounded-md bg-primary px-3 text-sm font-semibold text-white transition hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary/20 disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto sm:px-5"
      >
